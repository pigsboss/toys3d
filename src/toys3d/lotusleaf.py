#!/usr/bin/env python
import numpy as np
import trimesh
import os
from scipy.spatial import Delaunay
from collections import Counter

center_diameter = 145.0
skirt_width = 30.0
skirt_angle = np.deg2rad(60.0)
skirt_radius = skirt_width / np.cos(skirt_angle)

num_spokes = 12
thickness = 3.0
skirt_amplitude = 5.0

num_radius = 200
num_azimuth = 1000

rmin = center_diameter * 0.5
rmax = rmin + skirt_width
rgv = np.linspace(rmin, rmax, num_radius)
agv = np.arange(num_azimuth) / num_azimuth * 2.0 * np.pi
r, a = np.meshgrid(rgv, agv)
z = np.cos(a * num_spokes) * skirt_amplitude * r / rmax + (skirt_radius**2.0 - (r-rmin)**2.0)**0.5 - skirt_radius
x = np.cos(a) * r
y = np.sin(a) * r

v_top = np.vstack(([0., 0.,  thickness/2.], np.column_stack((x.ravel(), y.ravel(), z.ravel() + thickness/2.0))))
v_bot = np.vstack(([0., 0., -thickness/2.], np.column_stack((x.ravel(), y.ravel(), z.ravel() - thickness/2.0))))
r1 = np.arange(num_azimuth).ravel().astype('int32')
p1 = r1 * num_radius + 1
p2 = np.mod(r1+1, num_azimuth) * num_radius + 1
p0 = np.full_like(p1, 0)
offset = num_azimuth * num_radius + 1
tri_top_center = np.column_stack((p0, p1, p2))
tri_bot_center = np.column_stack((p0, p2, p1)) + offset

R, C = np.meshgrid(np.arange(num_azimuth).astype('int32'), np.arange(num_radius-1).astype('int32'), indexing='ij')
p0 = R * num_radius + C + 1
p1 = p0 + 1
p2 = np.mod(R+1, num_azimuth) * num_radius + C + 1
p3 = p2 + 1
tri_top_skirt_d = np.column_stack((p0.ravel(), p1.ravel(), p3.ravel()))
tri_top_skirt_u = np.column_stack((p0.ravel(), p3.ravel(), p2.ravel()))
tri_bot_skirt_d = np.column_stack((p0.ravel(), p2.ravel(), p1.ravel())) + offset
tri_bot_skirt_u = np.column_stack((p2.ravel(), p3.ravel(), p1.ravel())) + offset

r0 = np.arange(num_azimuth).ravel().astype('int32')
r1 = np.mod(r0+1, num_azimuth)
p0 = r0 * num_radius + num_radius
p1 = r1 * num_radius + num_radius

p2 = p0 + offset
p3 = p1 + offset
tri_side_u = np.column_stack((p0.ravel(), p2.ravel(), p1.ravel()))
tri_side_d = np.column_stack((p2.ravel(), p3.ravel(), p1.ravel()))

mesh = trimesh.Trimesh(
    np.vstack((v_top, v_bot)),
    np.vstack((
        tri_top_center, tri_bot_center,
        tri_top_skirt_u, tri_top_skirt_d,
        tri_bot_skirt_u, tri_bot_skirt_d,
        tri_side_u, tri_side_d)))
mesh.fix_normals()

if mesh.is_watertight:
    print(f'Watertight solid generated with {len(mesh.vertices)} vertices and {len(mesh.faces)} faces.')
else:
    print("Non-watertight mesh generated.")
    edge_counter = Counter()
    for face in mesh.faces:
        edge_counter[tuple(sorted([face[0], face[1]]))] += 1
        edge_counter[tuple(sorted([face[1], face[2]]))] += 1
        edge_counter[tuple(sorted([face[2], face[0]]))] += 1
    num_open_edges = 0
    num_complex_edges = 0
    num_kissing_edges = 0
    for e, cnt in edge_counter.items():
        if cnt < 2:
            num_open_edges += 1
        elif cnt > 2:
            num_complex_edges += 1
            if cnt == 4:
                num_kissing_edges += 1
            v1 = mesh.vertices[e[0]]
            v2 = mesh.vertices[e[1]]
    print(f"Diagnosis: {num_open_edges} open edges found, "
          f"{num_complex_edges} non-manifold edges found, "
          f"{num_kissing_edges} kissing edges found.")

mesh.export('lotusleaf.stl')
