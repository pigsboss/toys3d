...
# ------------------------------------------------------------------
#  新增：基于三角面片法向聚类的长方体主轴估计
# ------------------------------------------------------------------

def cluster_face_normals_box(mesh, n_clusters=6, antipodal=True, rng=None):
    """
    对长方体扫描网格的三角面法向做聚类，提取 3 对主方向。
    ...
    -------
    labels : (N,) ndarray
        面片聚类标签。若 antipodal=True，标签为 1..3；否则为 1..n_clusters。
    axes : list of ndarray
        聚类中心方向，antipodal=True 时返回 3 个方向。
    axes_weights : ndarray
        各轴向上的总面积/权重。
    """
    ...
    norms = np.linalg.norm(normals, axis=1, keepdims=True)
    valid = norms[:, 0] > 1e-12
    valid_normals = normals[valid] / norms[valid]
    valid_areas = areas[valid]

    labels_valid, centers = _spherical_kmeans_directions(
        valid_normals, weights=valid_areas, k=n_clusters, rng=rng
    )
    ...
    if not antipodal:
        labels[valid] = labels_valid + 1
        return labels, [normalize(c) for c in centers], \
               np.bincount(labels_valid, weights=valid_areas, minlength=n_clusters)
    ...
    area_per_cluster = np.bincount(labels_valid, weights=valid_areas,
                                   minlength=n_clusters)
    order = np.argsort(-area_per_cluster)

    new_id = 1
    for i in order:
        if i in used:
            continue
        dots = centers @ centers[i]
        dots[i] = -2
        for j in used:
            dots[j] = -2
        ...
        used.add(i)
        if j >= 0 and j not in used and np.dot(centers[i], centers[j]) < -0.5:
            used.add(j)
            axis = normalize(centers[i] - normalize(centers[j]))
            mask = (labels_valid == i) | (labels_valid == j)
            w = area_per_cluster[i] + area_per_cluster[j]
        else:
            axis = normalize(centers[i])
            mask = labels_valid == i
            w = area_per_cluster[i]

        labels[valid] = np.where(mask[labels_valid], new_id, labels[valid])
        merged_labels[valid] = np.where(mask, new_id, merged_labels[valid])
        axes.append(axis)
        weights.append(w)
        new_id += 1

    return merged_labels, axes, np.asarray(weights)


def _spherical_kmeans_directions(directions, weights=None, k=3,
                                 max_iter=300, tol=1e-6, rng=None):
    """
    对单位方向向量做面积/权重加权的球面 KMeans。
    ...
    dirs = np.where(dn > 1e-12, dirs / dn, dirs)
    N = len(dirs)
    ...
    centers[0] = dirs[idx]
    for i in range(1, k):
        dists = np.min(1.0 - np.abs(dirs @ centers[:i].T), axis=1)
        probs = weights * dists
        probs = np.maximum(probs, 0)
        if probs.sum() < 1e-12:
            centers[i] = dirs[rng.integers(N)]
        else:
            probs /= probs.sum()
            centers[i] = dirs[rng.choice(N, p=probs)]
    ...
    for _ in range(max_iter):
        ...
        for j in range(k):
            mask = labels == j
            if np.sum(weights[mask]) < 1e-12:
                continue
            c = dirs[mask].T @ weights[mask]
            norm = np.linalg.norm(c)
            if norm > 1e-12:
                new_centers[j] = c / norm
        shift = np.max(1.0 - np.sum(centers * new_centers, axis=1))
        centers = new_centers
        if shift < tol:
            break
...
def orthogonalize_box_axes(dirs, weights=None):
    """
    从 3 个近似正交的方向构造右手正交基。
    优先保留权重最大的轴。
    ...
    ux, uy, uz : 右手正交单位向量
    """
    dirs = [normalize(d) for d in dirs]
    if weights is None:
    ...
    d2 = dirs[order[1]]
    uy = normalize(d2 - np.dot(d2, ux) * ux)

    d3 = dirs[order[2]]
    uz_input = normalize(d3 - np.dot(d3, ux) * ux
                         - np.dot(d3, uy) * uy)
    uz = np.cross(ux, uy)
    if np.dot(uz, uz_input) < 0:
        uz = -uz
...
def compute_box_origin_extents(vertices, axes, method='bbox_center'):
    """
    把顶点投影到 3 个正交轴上，取中点作为原点，返回尺寸。

    Parameters
    ----------
    vertices : (N, 3)
    axes : (ux, uy, uz)
    method : 'bbox_center' | 'median'
    ...
    origin = center_local @ np.vstack([ux, uy, uz])
    return origin, extents


def signed_distance_to_box_planes(points, origin, axes, extents):
    """
    计算点到长方体 6 个面的有向距离。
    每个点返回 6 个距离（±x, ±y, ±z）。
    """
    ux, uy, uz = axes
    local = (np.asarray(points) - origin) @ np.vstack([ux, uy, uz]).T
...
def evaluate_box_fit(mesh, origin, axes, extents):
    """
    评估网格面心到拟合长方体表面的吻合程度。
    """
    centers = mesh.triangles_center
    dists = signed_distance_to_box_planes(centers, origin, axes, extents)
    min_abs = np.min(np.abs(dists), axis=1)
    areas = mesh.area_faces
    total_area = areas.sum()
    rmse = np.sqrt(np.sum(areas * min_abs**2) / total_area) if total_area > 0 else 0.0
    mad = float(np.median(min_abs))
    tol = 0.02 * np.max(extents)
    inlier_ratio = float(np.sum(areas[min_abs < tol]) / total_area) if total_area > 0 else 0.0
    ...
    return {
        ...
        'tol': tol,
        'max_abs_deviation': float(np.max(min_abs)),
    }


def build_box_aligned_frame_normal(mesh, n_clusters=6, rng=None):
    """
    基于三角面片法向聚类建立长方体局部正交坐标系。
    """
    labels, axes, weights = cluster_face_normals_box(
        mesh, n_clusters=n_clusters, antipodal=True, rng=rng
    )

    if len(axes) < 3:
        raise ValueError(f"Only {len(axes)} axes found, need 3 for a box.")

    ux, uy, uz = orthogonalize_box_axes(axes, weights[:3])
    origin, extents = compute_box_origin_extents(
        mesh.vertices, (ux, uy, uz), method='bbox_center'
    )
