...
def _generate_initial_seifert_disk(mesh, loop_vertices):
    """
    使用 SVD 平面投影 + 多边形内部规则网格 + `scipy.spatial.Delaunay`
    生成具有多个内部顶点的初始圆盘。
    ...
    # 确保边界为逆时针
    signed_area = 0.5 * sum(
        poly2d[i, 0] * poly2d[(i + 1) % len(poly2d), 1] -
        poly2d[(i + 1) % len(poly2d), 0] * poly2d[i, 1]
    ...
    boundary_count = len(poly2d)

    # 2. 在包围盒内生成规则网格候选点，保留多边形内部的点
    min_xy = poly2d.min(axis=0)
    max_xy = poly2d.max(axis=0)
    diag = float(np.linalg.norm(max_xy - min_xy))
    if diag < 1e-12:
        return None, []

    grid_resolution = max(int(np.ceil(np.sqrt(boundary_count))), 4)
    nx = grid_resolution + 1
    ny = grid_resolution + 1

    xs = np.linspace(min_xy[0], max_xy[0], nx)
    ys = np.linspace(min_xy[1], max_xy[1], ny)
    grid_x, grid_y = np.meshgrid(xs, ys)
    candidate_2d = np.column_stack([grid_x.ravel(), grid_y.ravel()])

    poly_list = [tuple(p) for p in poly2d]
    inside_flags = [
        _point_in_polygon_2d((px, py), poly_list)
        for px, py in candidate_2d
    ]
    interior_2d = candidate_2d[np.asarray(inside_flags, dtype=bool)]

    if len(interior_2d) < 1:
        # 边界太小时至少保留质心作内部点，避免退化
        interior_2d = poly2d.mean(axis=0, keepdims=True)

    # 3. 边界点 + 内部点，Delaunay 三角化
    from scipy.spatial import Delaunay

    points_2d = np.vstack([poly2d, interior_2d])
    tri = Delaunay(points_2d)

    kept_faces = []
    for simplex in tri.simplices:
        tri_pts_2d = points_2d[simplex]
        cx, cy = tri_pts_2d.mean(axis=0)
        if _point_in_polygon_2d((cx, cy), poly_list):
            kept_faces.append(simplex)

    if not kept_faces:
        return None, []

    faces = np.asarray(kept_faces, dtype=np.int64)

    # 4. 构造三维顶点
    vertices_3d = np.empty((len(points_2d), 3), dtype=np.float64)
    # 前 boundary_count 个顶点是边界点，直接用原始三维坐标
    vertices_3d[:boundary_count] = pts

    # 内部点按平面坐标映射回三维空间
    for idx in range(boundary_count, len(points_2d)):
        px, py = points_2d[idx]
        vertices_3d[idx] = centroid + px * u + py * v
...
def _laplacian_smooth_fixed_boundary(mesh, boundary_vertex_indices, iterations=200, step_size=1.0, tol=1e-7):
    """
    固定边界 Dirichlet 问题迭代求解 Plateau 问题。

    每轮迭代用当前几何重建余切 Laplacian，
    固定边界顶点，求解内部顶点线性系统：
        L_int * X_int = -L_int_boundary * X_boundary
    并通过 step_size 做松弛更新。
    """
    from scipy.sparse.linalg import spsolve
